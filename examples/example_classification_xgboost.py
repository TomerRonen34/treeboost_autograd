...
def main():
    ...
    custom_objective = XgboostObjective(loss_function=soft_margin_loss)
    model = XGBClassifier(objective=custom_objective, n_estimators=100)
    model.fit(X_train, y_train, eval_metric="error", eval_set=[(X_train, y_train)])
